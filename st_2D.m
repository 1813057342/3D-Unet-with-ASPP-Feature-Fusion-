...
function [st,t,f] = st_2D(timeseries,minfreq,maxfreq,samplingrate,freqsamplingrate)
...
% verbose = TRUE; 
verbose = FALSE;    
% removeedge= FALSE;%original
removeedge= TRUE;
% analytic_signal =  FALSE;
analytic_signal =  TRUE;
% factor = 1;%original
factor =1; %×Ô¶¨Òå factor=k.^2
...
st(banana/freqsamplingrate+1,:)=ifft(vector_fft(minfreq+banana+1:minfreq+banana+n).*g_window(n,minfreq+banana,factor));%original
% st(banana/freqsamplingrate+1,:)=ifft(vector_fft(minfreq+banana+1:minfreq+banana+n).*g_window(n,minfreq+banana,factor))*(banana);
end
...
%  note that the window here is defined in frequency domain
fac=1;
vector(1,:)=[0:length-1];
vector(2,:)=[-length:-1];
vector=vector.^2;  
vector=vector*(-factor*2*pi^2/(freq.^fac).^2); 
